...
const CartItem = ({item}) => {
  ...
  return (
    <div className='flex gap-4 border mb-10 rounded-lg p-4'>
        <img src={item.photo} alt=""  className='w-[115px] rounded-lg'/>
        <div className='w-full flex flex-col justify-between'>
            ...
            <div className='flex justify-between items-center'>
                ...
                <div className='border text-xl rounded-lg'>
                    <button onClick={handleDecrease} className='p-2 text-red-500 hover:bg-red-100'>
                       {item.amount > 1 ? <FaMinus/> :<FaTrash/>} 
                    </button>
                    <span className='p-3'>{item.amount}</span>
                    <button onClick={handleIncrease} className='p-3 rounded-lg text-red-500 hover:bg-red-100 transition'>
                        <FaPlus/>
                        </button>
                </div>
            </div>
